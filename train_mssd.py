...
from pasd.dataloader.localdatasets_drsr import LocalImageDataset, np2Tensor
...
def log_validation(vae, text_encoder, tokenizer, unet, controlnet, args, accelerator, weight_dtype, step):
    ...
    # test_dir = "/data0/llj/dataset/AID/real_world/test_HR"
    # for img_n in os.listdir(test_dir):
    #     img_p = os.path.join(test_dir, img_n)
    #     image = Image.open(img_p).convert("RGB")

    #     GT_image_t = np2Tensor(np.asarray(image)).unsqueeze(0).unsqueeze(0) # image
    #     LR_image_t, bkernel = degrade(GT_image_t, random=False)  # degradation LR_image_t [1, 1, 3, 200, 200]
    #     LR_image_t = LR_image_t.squeeze() / 255.0
    #     GT_image_t = GT_image_t.squeeze() / 255.0

    #     conditioning_pixel_values = LR_image_t
    #     pixel_values = GT_image_t * 2.0 - 1.0 
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet_q = ControlNetModel.from_unet(unet)
        controlnet_k = ControlNetModel.from_unet(unet)
        ...
        def save_model_hook(models, weights, output_dir):
            ...
            for i, model in enumerate(models):
                ...
                if sub_dir == "unet":
                    ...
                    model.encoder_q.save_pretrained(os.path.join(output_dir, sub_dir))
    ...
    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            ...
            model.encoder_q.enable_xformers_memory_efficient_attention()
            model.encoder_k.enable_xformers_memory_efficient_attention()
        else:
    ...
    if args.gradient_checkpointing:
        ...
        model.encoder_q.enable_gradient_checkpointing()
    ...
    if accelerator.unwrap_model(model.encoder_q).dtype != torch.float32:
        raise ValueError(
            f"Controlnet loaded as datatype {accelerator.unwrap_model(model.encoder_q).dtype}. {low_precision_error_string}"
        )
    ...
    # Optimizer creation
    params_to_optimize = list(model.encoder_q.parameters()) + list(unet.parameters())
    ...
    if True:
        ...
        train_dataset = LocalImageDataset(pngtxt_dir="/data0/llj/dataset/AID/real_world/train_HR", image_size=args.resolution, tokenizer=tokenizer, accelerator=accelerator, control_type=args.control_type, null_text_ratio=0.5, resize_bak=True)
    ...
    train_dataloader, unet, model, optimizer, lr_scheduler = accelerator.prepare(
        train_dataloader, unet, model, optimizer, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model.encoder_q),  accelerator.accumulate(model.encoder_k), accelerator.accumulate(unet):
                pixel_values, text, input_ids, conditioning_pixel_values = batch # pixel_values[4, 3, 64, 64], conditioning_pixel_values[4, 3, 64, 64]
                ...
                controlnet_cond_mid, down_block_res_samples, mid_block_res_sample = model.encoder_q(
                    noisy_latents, # float16
                    timesteps,
                    encoder_hidden_states=encoder_hidden_states,
                    controlnet_cond=controlnet_image,
                    return_dict=False
                )
                # noisy_latents [2, 4, 75, 75], timesteps [2], encoder_hidden_states [2, 77, 768], controlnet_image [2, 3, 600, 600]
                # controlnet_cond_mid LIST=3 [[2, 3, 300, 300], [2, 3, 150, 150], [2, 3, 75, 75]] 用于计算重建loss
                # down_block_res_samples  LIST=12 [[2, 320, 75, 75], [2, 320, 75, 75],[2, 320, 75, 75]...[2, 1280, 10, 10]] 
                # mid_block_res_sample LIST=2 [[1280, 10, 10],[1280, 10, 10]]
                ...
                # 计算moco loss
                # hr作为key
                controlnet_cond_mid_k, down_block_res_samples_k, mid_block_res_sample_k = model.encoder_k(
                    noisy_latents, # float16
                    timesteps,
                    encoder_hidden_states=encoder_hidden_states,
                ...
                output, target = model(im_q=mid_block_res_sample, im_k=mid_block_res_sample_k)
                moco_loss = criterion(output, target) * 0.1

                accelerator.backward(loss + moco_loss)
                if accelerator.sync_gradients:
                    params_to_clip = list(model.parameters()) + list(unet.parameters())
            ...
            logs = {"loss": loss.detach().item(), "moco": moco_loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
    ...
    if accelerator.is_main_process:
        controlnet = accelerator.unwrap_model(model.encoder_q)
