...
class BatchBlur(nn.Module):
    ...
    def forward(self, input, kernel): # input [1, 6, 200, 200], kernel [1, 21, 21]
        B, C, H, W = input.size()
        input_pad = self.pad(input) # [1, 6, 220, 220]
        H_p, W_p = input_pad.size()[-2:] # 220
        ...
        if len(kernel.size()) == 2:
            ...
            input_CBHW = input_pad.view((1, C * B, H_p, W_p)) # [1, 6, 220, 220]
            kernel = kernel.contiguous().view((B, 1, self.kernel_size, self.kernel_size))
            kernel = kernel.repeat(1, C, 1, 1).view((B * C, 1, self.kernel_size, self.kernel_size)) # [6, 1, 21, 21]
...
class SRMDPreprocessing(object):
    ...
    def __call__(self, hr_tensor, random=True): #hr_tensor[1, 2, 3, 200, 200]
        with torch.no_grad():
            # only downsampling
            ...
            # gaussian blur + downsampling
                ...
                B, N, C, H, W = hr_tensor.size()
                b_kernels = self.gen_kernel(B, random)  # B degradations b_kernels[1, 21, 21]

                # blur
                hr_blured = self.blur(hr_tensor.view(B, -1, H, W), b_kernels) # [1, 6, 200, 200]
            ...
            # bicubic 到原始图片大小
            lr_blured = F.interpolate(lr_blured, scale_factor=4, mode='bicubic', align_corners=False)

            # return lr_blured.view(B, N, C, H//int(self.scale), W//int(self.scale)), b_kernels
            return lr_blured.view(B, N, C, H, W), b_kernels
