    ...
    def __getitem__(self, index):
        ...
        if self.control_type is not None:
            if self.control_type == 'realisr':
                GT_image_t, LR_image_t = self.degradation.degrade_process(np.asarray(image)/255., resize_bak=self.resize_bak) # image(64, 64), GT_image_t[1, 3, 64, 64], LR_image_t[1, 3, 64, 64]
